...
sealed trait StreamAnswer[+A] {
  ...
  def takeWhile(p: A => Boolean): StreamAnswer[A] = {
    ...
    @scala.annotation.tailrec
    def loop(stream: StreamAnswer[A], taken: List[A] = List()): StreamAnswer[A] = {
      stream match {
        case EmptyStreamAnswer => listToStream(taken)
        case ConsStreamAnswer(h, t) if p(h()) => loop(t(), h() :: taken)
        case ConsStreamAnswer(h, t) if !p(h()) => loop(t(), taken)
      }
    }
  ...
  def scanRight[B](z: B)(f: (A, => B) => B): StreamAnswer[B] = {
    foldRight((z, StreamAnswer.cons(z, StreamAnswer.empty)))((a, p0) => {
      val b2 = f(a, p0._1)
      (b2, StreamAnswer.cons(b2, p0._2))
    })._2
  }
}
